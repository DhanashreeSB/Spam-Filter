...
{	
	...
	public NaiveBayes()
	{
		this.hamKeywords = new HashSet<Wordc>();
		this.spamKeywords = new HashSet<Wordc>();
		this.FeatureRepository = new ArrayList<Wordc>();
	...
	public boolean classify(String msg)
	{
		...
		{
			Wordc w = FeatureRepository.get(i);
	...
	//increase in inham i.e no. of times word appears in ham messages increases
		...
		try
		{
			...
			for(File f :files)
			{
				...
				while(line!=null)
				{
					...
					while(tok.hasMoreTokens())
					{
						Wordc w = new Wordc(tok.nextToken());
	...
	//increase in inspam i.e no. of times word appears in spam messages increases
	...
	private void loadHamKeywords(String hamFeaturesFile)
	{
		try
		{
			...
			while(word != null)
			{
				if(!word.trim().equals("")) hamKeywords.add(new Wordc(word.trim()));
				word = br.readLine();
			}
	...
	private void loadSpamKeywords(String spamFeaturesFile)
	{
		try
		{
			...
			while(word != null)
			{
				if(!word.trim().equals("")) spamKeywords.add(new Wordc(word.trim()));
				word = br.readLine();
			}
